...
def resize_model_pos_embed(model, img_size, encoder_global_attn_indexes):
    ...
    if img_size not in supported_sizes:
        raise ValueError(f"Image size {img_size} is not supported, only {supported_sizes} are supported")
...
def build_sam_adapter_linknet(
    ...
    if image_size not in supported_sizes:
        raise ValueError(f"Image size {image_size} is not supported, only {supported_sizes} are supported")
